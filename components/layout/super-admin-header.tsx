...
export function SuperAdminHeader() {
  ...
  // --- START: New logic for flicker-free hover ---
  ...
  // --- END: New logic for flicker-free hover ---
  ...
  const activeModuleNames = new Set(
    permittedSubmodules.map(sub => sub.module_name)
  );
            ...
            {allModules.map((mod) => {
              ...
              if (hasSubmodules) {
                return (
                  <div
                    key={mod.id}
                    onMouseEnter={() => handleMouseEnter(mod.module_name)}
                    onMouseLeave={handleMouseLeave}
                  >
                    <Popover open={hoveredCategory === mod.module_name} onOpenChange={(open) => { if (!open) setHoveredCategory(null); }}>
                      ...
                      <PopoverContent className="w-[500px] p-6 grid grid-cols-2 gap-4">
                        {(permittedSubmodules)
                          .filter((sub) => sub.module_name === mod.module_name)
                          .map((sub) => (
                            <div key={sub.id} className="space-y-1">
                              <Link href={`/${mod.module_name.toLowerCase()}/${sub.submodule_name.toLowerCase()}`} className="font-semibold hover:text-primary block">
                                {sub.submodule_name}
                              </Link>
                              <ul className="pl-2 space-y-1">
                                {(data?.functionalities?.results || [])
                                  .filter((fn) => fn.submodule_name === sub.submodule_name)
                                  .map((fn) => (
                                    <li key={fn.id}>
                                      <Link href={`/${mod.module_name.toLowerCase()}/${sub.submodule_name.toLowerCase()}/${fn.functionality_name.toLowerCase()}`} className="hover:text-primary text-sm block">
              ...
              return (
                <Button key={mod.id} variant="ghost" disabled className="text-sm font-medium shrink-0 opacity-75 cursor-default">
                  {mod.module_name}
                </Button>
              );
            })}
