    ...
    <Accordion type="single" collapsible className="w-full">
      {modules.map((modulePriv) => {
        ...
        const canLoadMoreData =
          hasNextPage && allSubmodulesForModule.length <= currentVisibleCount;
          ...
          <AccordionItem key={moduleId} value={`module-${moduleId}`}>
            <AccordionTrigger className="font-semibold hover:no-underline px-4">
              <div className="flex items-center justify-between w-full">
                ...
                {typeof handleModuleViewToggle === "function" && (
                  <div className="ml-4" onClick={(e) => e.stopPropagation()}>
                    <Checkbox
                      ...
                      onCheckedChange={(v) => {
                        if (!editMode) return;
                        handleModuleViewToggle(modulePriv, !!v);
                      }}
                      disabled={!!isModuleLoading || !editMode}
                      className="h-4 w-4"
                    />
                  </div>
                )}
            ...
            {!editMode && (
              <div className="px-4 text-xs text-muted-foreground">
                Enable "Manage Privileges" to edit module permissions.
              </div>
            )}
